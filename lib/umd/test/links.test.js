        ...
        var testFindLinksFor = function (value, expected) {
            ...
            return ls.findLinks(document, jsonDoc).then(function (list) {
                if (expected) {
                    ...
                    assert.deepEqual(list[0].range, jsonLanguageService_1.Range.create(document.positionAt(expected.offset), document.positionAt(expected.offset + expected.length)));
                }
                else {

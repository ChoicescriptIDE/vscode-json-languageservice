...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "assert", "../parser/jsonParser", "../jsonLanguageService"], factory);
    }
    ...
    suite('JSON Parser', function () {
        ...
        function toDocument(text) {
            ...
            var ls = jsonLanguageService_1.getLanguageService({});
            var jsonDoc = ls.parseJSONDocument(textDoc);
            return { textDoc: textDoc, jsonDoc: jsonDoc };
        }
        function toRange(text, offset, length) {
            var textDoc = jsonLanguageService_1.TextDocument.create('foo://bar/file.json', 'json', 0, text);
            return jsonLanguageService_1.Range.create(textDoc.positionAt(offset), textDoc.positionAt(offset + length));
        }
        ...
        test('Objects', function () {
            ...
            isInvalid('{3:3}');
            isInvalid('{\'key\': 3}');
            isInvalid('{"key" 3}', jsonLanguageService_1.ErrorCode.ColonExpected);
            isInvalid('{"key":3 "key2": 4}', jsonLanguageService_1.ErrorCode.CommaExpected);
            isInvalid('{"key":42, }', jsonLanguageService_1.ErrorCode.TrailingComma);
            isInvalid('{"key:42', jsonLanguageService_1.ErrorCode.UnexpectedEndOfString, jsonLanguageService_1.ErrorCode.ColonExpected);
        });
        test('Arrays', function () {
            ...
            isInvalid('[');
            isInvalid('[,]', jsonLanguageService_1.ErrorCode.ValueExpected);
            isInvalid('[1 2]', jsonLanguageService_1.ErrorCode.CommaExpected);
            isInvalid('[true false]', jsonLanguageService_1.ErrorCode.CommaExpected);
            isInvalid('[1, ]', jsonLanguageService_1.ErrorCode.TrailingComma);
            isInvalid('[[]', jsonLanguageService_1.ErrorCode.CommaOrCloseBacketExpected);
            isInvalid('["something"');
            isInvalid('[magic]');
        });
        test('Strings', function () {
            ...
            isInvalid('["\\u123Z"]');
            isInvalid('[\'string\']');
            isInvalid('"\tabc"', jsonLanguageService_1.ErrorCode.InvalidCharacter);
        });
        ...
        test('Simple AST', function () {
            {
                ...
                assert.equal(node.type, 'object');
                assert.deepEqual(jsonParser_1.getNodePath(node), []);
            ...
            }
            {
                ...
                assert.deepEqual(jsonParser_1.getNodePath(node), [0]);
            }
            {
                ...
                assert.deepEqual(jsonParser_1.getNodePath(node), ['a']);
        ...
        test('Missing colon', function () {
            ...
            var jsonDoc = toDocument(content).jsonDoc;
            assert.equal(jsonDoc.syntaxErrors.length, 1);
            assert.equal(jsonDoc.syntaxErrors[0].code, jsonLanguageService_1.ErrorCode.ColonExpected);
        ...
        test('Missing comma', function () {
            ...
            var jsonDoc = toDocument(content).jsonDoc;
            assert.equal(jsonDoc.syntaxErrors.length, 1);
            assert.equal(jsonDoc.syntaxErrors[0].code, jsonLanguageService_1.ErrorCode.CommaExpected);
        ...
        test('const', function () {
            ...
            }
            {
                ...
                var semanticErrors = jsonDoc.validate(textDoc, schema);
                assert.strictEqual(semanticErrors.length, 1);
                assert.strictEqual(semanticErrors[0].code, jsonLanguageService_1.ErrorCode.EnumValueMismatch);
            }
            {
                var schema_1 = {
                    properties: {
        ...
        test('multipleOf with floats', function () {
            ...
            {
                var _a = toDocument('[0.0002,0.2,0.64,2e+6,2.2e+10]'), textDoc = _a.textDoc, jsonDoc = _a.jsonDoc;
                var semanticErrors = jsonDoc.validate(textDoc, schema);
                assert.strictEqual(semanticErrors.length, 0);
            }
            {
                var _b = toDocument('[2e-5,2e-10,1e-4]'), textDoc = _b.textDoc, jsonDoc = _b.jsonDoc;
                var semanticErrors = jsonDoc.validate(textDoc, schema);
                assert.strictEqual(semanticErrors.length, 3);
            }
            schema = {
            ...
            {
                var _c = toDocument('[2.000000001e5,6.000000003e8]'), textDoc = _c.textDoc, jsonDoc = _c.jsonDoc;
                var semanticErrors = jsonDoc.validate(textDoc, schema);
                assert.strictEqual(semanticErrors.length, 0);
            }
        });
        ...
        test('Strings with spaces', function () {
            ...
            assert.strictEqual(jsonParser_1.getNodeValue(node), 'first string');
            node = jsonDoc.getNodeFromOffset(34);
            assert.strictEqual(jsonParser_1.getNodeValue(node), 'second string');
        });
        test('Schema information on node', function () {
            ...
            assert.strictEqual(jsonParser_1.getNodeValue(node), 42);
        ...
        test('parse with comments', function () {
            function parse(v) {
                ...
                assert.equal(jsonDoc.syntaxErrors.length, 0);
                return jsonParser_1.getNodeValue(jsonDoc.root);
            }
